...
class CommandExecutor(object):
    ...
    def Execute(self, command_to_execute, raise_exception_on_failure=False, communicator=None, input=None, suppress_logging=False, timeout=0):
        if not suppress_logging:
            print("Executing: {0}".format(command_to_execute))
        ...
        try:
            ...
            if raise_exception_on_failure:
                ...
                if not suppress_logging:
                    ...
                    print("Process creation failed: " + traceback.format_exc())
                return -1

        def timeout_process():
            proc.kill()
            print("Command {0} didn't finish in {1} seconds. Timing it out".format(command_to_execute, timeout))
