...
from AbstractForkingDaemon import AbstractForkingDaemon
from Common import CommonVariables
from ProcessLock import ProcessLock

class ResumeOnlineEncryptionDaemon(AbstractForkingDaemon):
    ...
    def _get_lock_file_path(self):
        return self.disk_util.encryption_environment.resume_daemon_lock_file_path
    ...
    def run(self):
        ...
        lock = ProcessLock(logger, self._get_lock_file_path())
        if not lock.try_lock():
            logger.log("there's another ResumeOnlineEncryptionDaemon running, please wait it to exit.", level = CommonVariables.WarningLevel)
            return
        ...
        try:
            ...
            lock.release_lock()
            logger.log("exiting ResumeOnlineEncryptionDaemon")
            sys.exit(0)
