...
class UnmountOldrootState(OSEncryptionState):
    ...
    def enter(self):
        ...
        self.command_executor.ExecuteInBash('systemctl daemon-reexec', True)
        self.command_executor.ExecuteInBash('systemctl restart \\"*\\"', False)
        # self.command_executor.ExecuteInBash('systemctl restart \\"*\\"', True)
        self.command_executor.ExecuteInBash('pkill -f omsagent.py')
        sleep(60)
        ...
        self.command_executor.Execute('umount /oldroot/tmp/tmproot')
        self.command_executor.Execute('umount -r /oldroot')
        ...
        self.command_executor.Execute('fsck -y {0}'.format(self.rootfs_block_device), True)
