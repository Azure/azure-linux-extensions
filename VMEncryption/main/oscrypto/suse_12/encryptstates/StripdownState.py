...
class StripdownState(OSEncryptionState):
    ...
    def enter(self):
        ...
        self.command_executor.ExecuteInBash('for i in bin sbin lib lib64 share ; do cp -ax /usr/$i /tmp/tmproot/usr/; done', True)
        # self.command_executor.ExecuteInBash('for i in bin sbin libexec lib lib64 share; do cp -ax /usr/$i /tmp/tmproot/usr/; done', True)
        self.command_executor.ExecuteInBash('for i in lib opt run tmp; do cp -ax /var/$i /tmp/tmproot/var/; done', True)
    ...
    # for i in proc sys dev run usr var tmp root oldroot boot; do mkdir / tmp / tmproot / $i; done
    # for i in bin etc mnt sbin lib lib64 root; do cp -ax / $i / tmp / tmproot /; done
    # for i in bin sbin lib lib64 share; do cp -ax / usr / $i / tmp / tmproot / usr /; done
    # for i in lib opt run tmp; do cp -ax / var / $i / tmp / tmproot / var /; done
