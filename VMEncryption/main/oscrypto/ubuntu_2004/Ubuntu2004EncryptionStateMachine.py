...
class Ubuntu2004EncryptionStateMachine(OSEncryptionStateMachine):
    states = [
    ...
    def on_enter_state(self):
        super(Ubuntu2004EncryptionStateMachine, self).on_enter_state()

    def should_exit_previous_state(self):
        # when this is called, self.state is still the "source" state in the transition
        return super(Ubuntu2004EncryptionStateMachine, self).should_exit_previous_state()

    def __init__(self, hutil, distro_patcher, logger, encryption_environment):
        super(Ubuntu2004EncryptionStateMachine, self).__init__(hutil, distro_patcher, logger, encryption_environment)
        ...
        self.state_machine = Machine(model=self,
                                     states=Ubuntu2004EncryptionStateMachine.states,
                                     transitions=Ubuntu2004EncryptionStateMachine.transitions,
