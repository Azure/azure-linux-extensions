...
class StripdownState(OSEncryptionState):
    ...
    def enter(self):
        ...
        if self._is_arm64():
            self.command_executor.ExecuteInBash('for i in bin etc mnt sbin lib root; do cp -ax /$i /tmp/tmproot/; done', True)
            self.command_executor.ExecuteInBash('for i in bin sbin lib local share; do cp -ax /usr/$i /tmp/tmproot/usr/; done', True)
        else:
