...
class DiskUtil(object):
    ...
    def get_device_items_property(self, lsblk_path, dev_name, property_name):
        ...
        get_property_cmd_args =Utils.HandlerUtil.HandlerUtility.split(get_property_cmd)
        ...
        for i in range(0,len(lines)):
            ...
            if(item_value_str != ""):
                disk_info_item_array =Utils.HandlerUtil.HandlerUtility.split(item_value_str)
    ...
    def get_device_items_sles(self,dev_path):
        ...
        get_device_cmd_args =Utils.HandlerUtil.HandlerUtility.split(get_device_cmd)
    ...
    def get_mount_points(self):
        ...
        if (out_mount_output is not None):
            ...
            for line in lines:
                line = line.strip()
                if(line != ""):
                    deviceName =Utils.HandlerUtil.HandlerUtility.split(line)[0]
                    mountPrefixStr = " on /"
    ...
    def get_mount_file_systems(self):
        ...
        if (out_mount_output is not None):
            ...
            for line in lines:
                ...
                line = line.strip()
                if(line != ""):
                    file_system =Utils.HandlerUtil.HandlerUtility.split(line)[0]
                    mountPrefixStr = " on /"
                    prefixIndex = line.find(mountPrefixStr)
                    if(prefixIndex >= 0):
                        mountpointStart = prefixIndex + len(mountPrefixStr) - 1
